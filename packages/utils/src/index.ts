  ...
  */
  ...
  getTreeNode(Data: any[], childrenKey: string, key: string, value: any): any {
    ...
    for (F = Data.length; F; ) {
      ...
      if (T[childrenKey]) {
        Deep = Utils.getTreeNode(T[childrenKey], childrenKey, key, value);
        if (Deep) return Deep;
      }
    }
  },
  ...
  array2Tree(
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (node.parentId === parentId) {
        const children = Utils.array2Tree(data, node.id, keyName);
        const newNode: any = {
          ...node,
          [keyName]: node.id,
          children,
   ...
   */
  getFileUnit(size: number | string): string {
  ...
  // 获取两个时间的间隔描述
  getTimeInterval(startTimeStr: string, endTimeStr: string): string {
