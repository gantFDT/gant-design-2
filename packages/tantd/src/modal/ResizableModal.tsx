...
import { SwitcherOutlined, BorderOutlined } from '@ant-design/icons';
...
const ModalInner: React.FC<InnerModalProps> = function ModalInner(props) {
  ...
  const titleElement = useMemo(
    () => (
      <div className={classnames(`${prefixCls}-resizableModalTitle`, isMaximized ? '' : `${prefixCls}-canDrag`)} style={{ marginRight: canMaximize ? 70 : 30 }} onMouseDown={onMouseDrag} onClick={onFocus} onDoubleClick={toggleMaximize}>
        {title}
      </div>
    ),
  ...
  const combineWrapClassName = useMemo(() => {
    return classnames(`${prefixCls}-resizableModalWrapper`, isModalDialog ? `${prefixCls}-resizableModalDialog` : `${prefixCls}-resizableModalDefault`, isMaximized && `${prefixCls}-maximize`, wrapClassName);
  }, [isMaximized, isModalDialog]);

  return (
    <Modal wrapClassName={combineWrapClassName} title={title && titleElement} width={width} visible={modalVisible} zIndex={zIndex} style={_style} mask={isModalDialog} maskClosable={isModalDialog} destroyOnClose onCancel={onCancel} onOk={onOk} cancelButtonProps={{ size: 'small', ...cancelButtonProps }} okButtonProps={{ size: 'small', ...okButtonProps }} {...restProps}>
      {/*弹窗内容 */}
      ...
      {/*最大化按钮 */}
      {canMaximize && (
        <div className={`${prefixCls}-maximizeAnchor`} onClick={toggleMaximize}>
