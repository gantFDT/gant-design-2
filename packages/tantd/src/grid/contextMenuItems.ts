import type { GetContextMenuItemsParams, RowNode } from '@ag-grid-community/core';
import type { DefaultJsonParams } from './interface';
...
export const gantGetcontextMenuItems = function (params: GetContextMenuItemsParams, config: ContextMenuItemsConfig) {
  const { downShift, locale, onRowsCut, onRowsPaste, getContextMenuItems, defaultJsonParams = {} } = config;
  const {
  ...
  if (node) {
    ...
    if (!downShift || rowNodes.length == 0) {
      ...
      if (!minIndex || !maxIndex) return; //娄底
