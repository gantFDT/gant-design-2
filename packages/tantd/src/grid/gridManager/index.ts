import type { RowDataTransaction, GridApi, RowNode, ColDef, ColGroupDef, ColumnApi } from '@ag-grid-community/core';
...
import type { AgGridConfig, OperationAction, Diff, BatchUpdateDataSourceParams } from './interface';
...
export default class GridManage {
  ...
  async validate(data?: any[]) {
    ...
    let source: any = isEmpty(data) ? [...add, ...modify] : data;
    const fields: any = {};
    ...
    source = source.map((item, index) => {
      ...
      Object.keys(validateFields).forEach((validateField) => {
    ...
    try {
      ...
      errors.forEach((itemError) => {
        const [, index, field] = itemError.field.split('.');
        const fieldName = field.replace(/-/g, '.');
        const nodeId = getRowNodeId(get(source, `[${index}]`, {}));
        const rowNode = this.agGridApi.getRowNode(nodeId);
        const { message } = itemError;
        if (rowNode) {
          this.getNodeExtendsParent(rowNode);
          const { rowIndex } = rowNode;
          if (Reflect.has(validateErros, rowIndex)) {
            validateErros[rowIndex].push({ field: fieldName, message });
          } else {
            validateErros[rowIndex] = [{ field: fieldName, message }];
          }
          nodeIds = [...nodeIds, nodeId];
          nodeFields = [...nodeFields, fieldName];
        }
      });
  ...
  errorSign(validateErros: any, newData: any[]) {
    ...
    newData.forEach((itemData) => {
      ...
      if (errorsArr) {
        const _rowError: any = {};
        errorsArr.forEach((itemError) => {
          _rowError[itemError.field] = itemError.message;
        });
        rowNode.setData({ ...newItemData, _rowError });
      } else if (merge_rowError) rowNode.setData({ ...newItemData });
    });
  }

  private getNodeExtendsParent(rowNode: RowNode | null, first = true) {
    if (!rowNode) return; //娄底
    if (rowNode.level > 0) {
      this.getNodeExtendsParent(rowNode.parent, false);
      if (!rowNode.parent) return; //娄底
  ...
  paste(node, up = true) {
    try {
      ...
      if (!treeData) {
        ...
        if (!createConfig || !canQuickCreate(createConfig)) return console.warn('createConfig is error'); //娄底
        let { defaultParentPath = [] } = createConfig;
        defaultParentPath = Array.isArray(defaultParentPath) ? defaultParentPath : [];
        let parentPath: any = !node ? defaultParentPath : [];
        if (node) {
          const brotherPath = getDataPath && getDataPath(get(node, 'data', [])); //娄底
          parentPath = brotherPath && brotherPath.slice(0, brotherPath.length - 1);
        }
  ...
  // 创建;
  @hisDecorator()
  public create(records: any, targetId?: boolean | string | string[] | number | number[], isSub: boolean = true) {
    const { getRowNodeId } = this.agGridConfig;
    ...
    if ((typeof targetId !== 'number' && !targetId) || typeof targetId === 'boolean') {
      ...
      this.agGridApi.setRowData(isFirst ? [...addRecords, ...rowData] : [...rowData, ...addRecords]);
    ...
    const targetArray = Array.isArray(targetId) ? targetId : [targetId];
    let hisRecords: any[] = [];
    const newRecords: any[] = [];
    targetArray.forEach((itemId, index) => {
  ...
  private quickCreateNode(isChild: boolean = false, targetId: string, record: number | object | any[] = 1) {
    ...
    const nodePath: any = getDataPath && getDataPath(nodeData);
    const parentPath = isChild ? [...nodePath] : [...nodePath.slice(0, nodePath.length - 1)];
    if (typeof record == 'number') {
      const len = record > 1 ? record : 1;
      const records: any[] = [];
      for (let index = 0; index < len; index++) {
        const data: any = {};
        const id = createUUID() + '';
        if (!createConfig) return; //娄底
    ...
    if (Array.isArray(record)) {
      const records = record.map((item) => {
        const id = createUUID() + '';
        if (!createConfig) return; //娄底
        const data = { ...item, [createConfig.id]: id };
        if (getDataPath && treeData) {
          data[createConfig.path] = createConfig.toPath([...parentPath, id]);
        }
        return data;
  ...
  private toggleUndoRedo(hisStack: OperationAction, undo: boolean = true) {
    ...
    if (type === DataActions.remove && recordsIndex) {
      recordsIndex.forEach((removeIndex, index) => {
      ...
      records.forEach((itemRecord) => {
        ...
        if (recordsIndex) recordsIndex.unshift(removeIndex); //娄底
      });
  ...
  @hisDecorator()
  undo() {
    const hisStack = this.historyStack.pop();
    if (isEmpty(hisStack)) return;
    const newhisStack: any = this.toggleUndoRedo(hisStack, true);
    this.redoStack.push(newhisStack);
  }
  @hisDecorator()
  redo() {
    const hisStack: any = this.redoStack.pop();
    if (isEmpty(hisStack)) return;
  ...
  private changeDiff() {
    ...
    nowHistoryStack.reverse().forEach((hisItem) => {
      const { type, recordsIndex, records } = hisItem;
      records.forEach((recordItem, recordItemIndex) => {
        ...
        const { _rowData: _rowDataOrigin, _rowType, _rowError, ...data } = _nextRowData;
        const _rowData = isEmpty(_rowDataOrigin) ? data : _rowDataOrigin;
    ...
    const diff: any = groupBy(diffArray, 'actionType');
  ...
  getPureData() {
    ...
    this.agGridApi.forEachNode(function (node, index) {
      const cloneData = cloneDeep(get(node, 'data', {}));
