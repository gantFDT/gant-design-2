...
const FlipOverFooter = (props: FlipOverFooterProps) => {
  ...
  const onFooterChangePage = useCallback(
    (parameter) => {
      const item = parameter == 'before' ? data[currentIndex - 1] : data[currentIndex + 1];
      if (item && onSelectedChange) onSelectedChange(item[itemKey], item);
    },
    [data, currentIndex],
  );

  return (
    <div className={classnames(`${prefixCls}-contextfooter`, className)} style={style}>
      <div className={`${prefixCls}-footerdiv`} onClick={onFooterChangePage.bind(null, 'before')} style={{ display: currentIndex == 0 ? 'none' : 'block' }}>
        <span style={{ display: 'inline-block', fontSize: 16, color: 'rgba(128,128,128,1)', margin: '0 5px' }}>
          <LeftCircleOutlined />
        </span>
        <p style={{ display: 'inline-block', color: 'rgba(128,128,128,1)', margin: 0 }}>{currentIndex == 0 ? '' : data[currentIndex - 1].title}</p>
      </div>
      <div className={`${prefixCls}-footerdiv`} onClick={onFooterChangePage.bind(null, 'after')} style={{ textAlign: 'right', display: currentIndex + 1 == data.length ? 'none' : 'block' }}>
